...
public class Entity {
  ...
  public boolean collides(Entity otherEntity) {
	...
	if (otherEntity instanceof DestructibleImageEntity) {
		
		DestructibleImageEntity collidableEntity = (DestructibleImageEntity) otherEntity;
		
		Vector2f position = this.getPosition();
		if (position == null) {
			return false; // a shot without a position cannot collide
		}
		...
		boolean collides = collidableEntity.collides(position.x - halfWidth, position.y - halfHeight);
		collides |= collidableEntity.collides(position.x + halfWidth, position.y - halfHeight);
		collides |= collidableEntity.collides(position.x - halfWidth, position.y + halfHeight);
		collides |= collidableEntity.collides(position.x + halfWidth, position.y + halfHeight);
